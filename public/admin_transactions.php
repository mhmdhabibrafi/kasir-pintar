...
$deleteTransactions = static function (PDO $pdo, array $ids): void {
    ...
    $refundIdsStmt = $pdo->prepare("SELECT id FROM refunds WHERE transaction_id IN ($placeholders)");
    $refundIdsStmt->execute($ids);
    $refundIds = array_map('intval', $refundIdsStmt->fetchAll(PDO::FETCH_COLUMN));

    if (!empty($refundIds)) {
        $refundPlaceholders = implode(',', array_fill(0, count($refundIds), '?'));
        $pdo->prepare("DELETE FROM refund_items WHERE refund_id IN ($refundPlaceholders)")->execute($refundIds);
    }

    $pdo->prepare("DELETE FROM refunds WHERE transaction_id IN ($placeholders)")->execute($ids);
    $pdo->prepare("DELETE FROM transaction_meta WHERE transaction_id IN ($placeholders)")->execute($ids);
    ...
    try {
        if (!empty($errors)) {
            ...
            if ($reason === '') {
                ...
                $pdo->exec('DELETE FROM refunds');
                $pdo->exec('DELETE FROM transaction_meta');
