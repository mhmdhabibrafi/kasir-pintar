...
function shift_add_movement(int $userId, string $type, float $amount, string $note = ''): void
{
    ...
    $pdo = db();
    $stmt = $pdo->prepare(
        'SELECT id, shift_code FROM shifts WHERE user_id = :user_id AND closed_at IS NULL ORDER BY opened_at DESC LIMIT 1'
    );
    $stmt->execute([':user_id' => $userId]);
    $row = $stmt->fetch();
    if (!$row) {
...
function shift_add_movement_by_code(string $shiftCode, string $type, float $amount, string $note = ''): bool
{
    ...
    $pdo = db();
    $stmt = $pdo->prepare(
        'SELECT id FROM shifts WHERE shift_code = :code AND closed_at IS NULL LIMIT 1'
    );
    $stmt->execute([':code' => $shiftCode]);
